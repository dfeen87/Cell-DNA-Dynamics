...
(Eq. 7).  All kernels are defined on the half-open interval s ∈ [0, ∞) and
are identically zero for s < 0 (strict causality: only past values τ < t
contribute to χ(t)).
...
gaussian_kernel(s, sigma)
    ...
    One-sided Gaussian weighting truncated to causal half-line.
...
def exponential_kernel(s: "np.ndarray | float", lam: float) -> np.ndarray:
    ...
    s : array-like or float
        Time lag values s = t − τ.  Must be non-negative for a causal
...
def gaussian_kernel(s: "np.ndarray | float", sigma: float) -> np.ndarray:
    ...
    K = np.where(s >= 0, norm * np.exp(-(s**2) / (2.0 * sigma**2)), 0.0)
    return K
